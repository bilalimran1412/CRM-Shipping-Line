...
function Searchbar() {
  ...
  const reduceItems = NavConfig(checkPermission).map((list) => handleLoop(list.items, list.subheader)).flat()

  const allItems = flattenArray(reduceItems).map((option) => {
    const group = splitPath(reduceItems, option.path)
    return {
      group: translate(group && group.length > 1 ? group[0] : option.subheader),
      title: translate(option.title),
      path: option.path,
      indexKey: 'minimal',
      ignore: option.ignoreSearchbar,
    }
  })
        ...
        <Slide direction="down" in={open} mountOnEnter unmountOnExit>
          <StyledSearchbar>
            <Autocomplete
              ...
              options={allItems.sort((a, b) => -b.group.localeCompare(a.group)).filter(item => {
                if(item.ignore === true){
                  return false
                }
                return true
...
function handleLoop(array, subheader = '') {
  return array?.map((list) => {
    if (list.permission !== undefined && list.permission === false) {
      return null
    }
    return {
      subheader,
