...
export default function PageOne() {
  ...
  const {fetchList, data, page, isLoading, deleteData, error} = useApi(store)
  ...
  const filters = (
    <>
      ...
      <FilterToolbar additionalFilters={['status']} arrayFields={['type']}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={4}>
            <RHFTextField
              ...
              placeholder={translate('group.filter.search')}
              InputProps={{
                startAdornment: (
  ...
  const onDelete = id => {
    confirmRef.current.open({
      title: translate('group.delete.title'),
      content: convertNewlinesToBreaks(translate('group.delete.description')),
      ...
      callback: (close) => {
        deleteData(id, {
          success: data => {
            enqueueSnackbar(translate('group.success.delete'), {
              variant: 'success',
          ...
          error: e => {
            enqueueSnackbar(translate('group.error.delete'), {
              variant: 'error',
              autoHideDuration: 5 * 1000
            })
            close()
  ...
  const tableCols = getCols({translate, onDelete, checkPermission})

  return (
    <>
      <Helmet>
        <title>{translate('group.title.page')}</title>
      </Helmet>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={translate('group.title.list')}
          links={[
            {name: translate('breadcrumb.main'), href: MAIN_NAVIGATION_ROOT},
            {name: translate('group.breadcrumb.main'), href: LIST_NAVIGATION},
            {name: translate('group.breadcrumb.list')},
          ]}
          action={
