...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    updateData(id, data, {
      success: data => {
        enqueueSnackbar(translate('vehicle-task-type.success.edit'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('vehicle-task-type.error.edit'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('vehicle-task-type.title.edit')}</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        ...
        <CustomBreadcrumbs
          heading={translate('vehicle-task-type.title.edit')}
          links={[
            ...
            {name: translate('vehicle-task-type.breadcrumb.edit')},
          ]}
        />
