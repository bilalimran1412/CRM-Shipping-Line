...
export default function PageOne() {
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <RHFTextField
              ...
              placeholder={translate('vehicle-task-type.filter.search')}
              InputProps={{
                startAdornment: (
  ...
  const onDelete = id => {
    confirmRef.current.open({
      title: translate('vehicle-task-type.delete.title'),
      content: convertNewlinesToBreaks(translate('vehicle-task-type.delete.description')),
      ...
      callback: (close) => {
        deleteData(id, {
          success: data => {
            enqueueSnackbar(translate('vehicle-task-type.success.delete'), {
              variant: 'success',
          ...
          error: e => {
            enqueueSnackbar(translate('vehicle-task-type.error.delete'), {
              variant: 'error',
              autoHideDuration: 5 * 1000
            })
            close()
  ...
  return (
    <>
      <Helmet>
        <title>{translate('vehicle-task-type.title.page')}</title>
      </Helmet>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={translate('vehicle-task-type.title.list')}
          links={[
            ...
            {name: translate('vehicle-task-type.breadcrumb.list')},
          ]}
          action={
