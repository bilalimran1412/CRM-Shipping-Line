...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    createData(data, {
      success: data => {
        enqueueSnackbar(translate('shipment-company.success.create'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('shipment-company.error.create'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('shipment-company.title.create')}</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        ...
        <CustomBreadcrumbs
          heading={translate('shipment-company.title.create')}
          links={[
            ...
            {name: translate('shipment-company.breadcrumb.create')},
          ]}
        />
