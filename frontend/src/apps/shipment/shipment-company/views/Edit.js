...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    updateData(id, data, {
      success: data => {
        enqueueSnackbar(translate('shipment-company.success.edit'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('shipment-company.error.edit'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('shipment-company.title.edit')}</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        ...
        <CustomBreadcrumbs
          heading={translate('shipment-company.title.edit')}
          links={[
            ...
            {name: translate('shipment-company.breadcrumb.edit')},
          ]}
        />
