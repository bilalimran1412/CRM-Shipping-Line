...
export default function PageOne() {
  ...
  const onDelete = id => {
    confirmRef.current.open({
      title: translate('shipment.delete.title'),
      content: convertNewlinesToBreaks(translate('shipment.delete.description')),
      ...
      callback: (close) => {
        deleteData(id, {
          success: data => {
            enqueueSnackbar(translate('shipment.success.delete'), {
              variant: 'success',
          ...
          error: e => {
            enqueueSnackbar(translate('shipment.error.delete'), {
              variant: 'error',
              autoHideDuration: 5 * 1000
            })
            close()
  ...
  return (
    <>
      <Helmet>
        <title>{translate('shipment.title.page')}</title>
      </Helmet>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={translate('shipment.title.list')}
          links={[
            {name: translate('breadcrumb.main'), href: MAIN_NAVIGATION_ROOT},
            {name: translate('shipment.breadcrumb.main'), href: LIST_NAVIGATION},
            {name: translate('shipment.breadcrumb.list')},
          ]}
          action={
        ...
        <Datatable
          collapse
          collapseRender={row => renderCollapse({data: (row?.vehicles || []), tableCols: collapseCols, themeMode, translate})}
          actions={actions}
          filters={filters}
          loading={isLoading}
          data={data}
