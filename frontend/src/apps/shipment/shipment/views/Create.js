...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    createData(data, {
      success: data => {
        enqueueSnackbar(translate('shipment.success.create'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('shipment.error.create'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('shipment.title.create')}</title>
      </Helmet>
