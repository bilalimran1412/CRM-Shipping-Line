...
export const getCols = ({translate, onClickAvatar, onDelete, checkPermission, currentLang, context}) => {
  ...
  return [
    ...
    {
      id: 'customer',
      label: translate('customer-invoice.table.customer'),
      align: 'left',
      width: '300px',
      render: row => {
        return (
    ...
    {
      id: 'template',
      label: translate('customer-invoice.table.template'),
      align: 'left',
      width: '300px',
      render: row => {
        return (
          <Typography component={'span'} variant="subtitle1" sx={{color: `warning.main`}}>
            {findTemplate(row.template)}
          </Typography>
        )
      },
    },
        ...
        return (
          <>
            <Typography variant="subtitle1" sx={{color: `${statusColor}.main`}}>
              {translate('customer-invoice.table.finance.status')}: {translate(`customer-invoice.table.finance.${row.status}`)}
            </Typography>
