import {
  ...
  const Payments = forwardRef(({formData}, ref) => {
        ...
        <Grid item xs={12} md={12}>
          <Card sx={{p: 3, height: '100%'}}>
            <Stack direction="row" alignItems="center" sx={{mb: 2}}>
              <Box sx={{flexGrow: 1}}>
                <Typography variant="h6" sx={{color: 'text.disabled'}}>
                  {translate('customer-invoice.form.payments')}
                </Typography>
              </Box>
              <Box>
                <Box sx={{flexShrink: 0}}>
            ...
            <Scrollbar>
              {data.map((row, index) => {
                return (
                  <Grid container spacing={3} className={'invoice-vehicle-form'} sx={{pb: 2, mt: 0, pl: 1, pr: 1}}
                        key={row._field_id}>
