...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    createData(data, {
      success: data => {
        enqueueSnackbar(translate('customer-invoice.success.create'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('customer-invoice.error.create'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('customer-invoice.title.create')}</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        ...
        <CustomBreadcrumbs
          heading={translate('customer-invoice.title.create')}
          links={[
            ...
            {name: translate('customer-invoice.breadcrumb.create')},
          ]}
        />
