...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    updateData(id, data, {
      success: data => {
        enqueueSnackbar(translate('vehicle.success.edit'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('vehicle.error.edit'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('vehicle.title.edit')}</title>
      </Helmet>
