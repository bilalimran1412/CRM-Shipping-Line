...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    createData(data, {
      success: data => {
        enqueueSnackbar(translate('delivery-status.success.create'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('delivery-status.error.create'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('delivery-status.title.create')}</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        ...
        <CustomBreadcrumbs
          heading={translate('delivery-status.title.create')}
          links={[
            ...
            {name: translate('delivery-status.breadcrumb.create')},
          ]}
        />
