...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    updateData(id, data, {
      success: data => {
        enqueueSnackbar(translate('delivery-status.success.edit'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('delivery-status.error.edit'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('delivery-status.title.edit')}</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        ...
        <CustomBreadcrumbs
          heading={translate('delivery-status.title.edit')}
          links={[
            ...
            {name: translate('delivery-status.breadcrumb.edit')},
          ]}
        />
