...
export default function PageOne() {
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <RHFTextField
              ...
              placeholder={translate('delivery-status.filter.search')}
              InputProps={{
                startAdornment: (
  ...
  const onDelete = id => {
    confirmRef.current.open({
      title: translate('delivery-status.delete.title'),
      content: convertNewlinesToBreaks(translate('delivery-status.delete.description')),
      ...
      callback: (close) => {
        deleteData(id, {
          success: data => {
            enqueueSnackbar(translate('delivery-status.success.delete'), {
              variant: 'success',
          ...
          error: e => {
            enqueueSnackbar(translate('delivery-status.error.delete'), {
              variant: 'error',
              autoHideDuration: 5 * 1000
            })
            close()
  ...
  return (
    <>
      <Helmet>
        <title>{translate('delivery-status.title.page')}</title>
      </Helmet>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={translate('delivery-status.title.list')}
          links={[
            {name: translate('breadcrumb.main'), href: MAIN_NAVIGATION_ROOT},
            {name: translate('delivery-status.breadcrumb.main'), href: LIST_NAVIGATION},
            {name: translate('delivery-status.breadcrumb.list')},
          ]}
          action={
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              >
                {translate('delivery-status.create')}
              </Button>
            </Stack>
          }
        />
