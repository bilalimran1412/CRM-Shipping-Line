...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    createData(data, {
      success: data => {
        enqueueSnackbar(translate('vehicle-photo-category.success.create'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('vehicle-photo-category.error.create'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('vehicle-photo-category.title.create')}</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        ...
        <CustomBreadcrumbs
          heading={translate('vehicle-photo-category.title.create')}
          links={[
            ...
            {name: translate('vehicle-photo-category.breadcrumb.create')},
          ]}
        />
