...
export default function PageOne() {
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <RHFTextField
              ...
              placeholder={translate('vehicle-photo-category.filter.search')}
              InputProps={{
                startAdornment: (
  ...
  const onDelete = id => {
    confirmRef.current.open({
      title: translate('vehicle-photo-category.delete.title'),
      content: convertNewlinesToBreaks(translate('vehicle-photo-category.delete.description')),
      ...
      callback: (close) => {
        deleteData(id, {
          success: data => {
            enqueueSnackbar(translate('vehicle-photo-category.success.delete'), {
              variant: 'success',
          ...
          error: e => {
            enqueueSnackbar(translate('vehicle-photo-category.error.delete'), {
              variant: 'error',
              autoHideDuration: 5 * 1000
            })
            close()
  ...
  return (
    <>
      <Helmet>
        <title>{translate('vehicle-photo-category.title.page')}</title>
      </Helmet>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={translate('vehicle-photo-category.title.list')}
          links={[
            {name: translate('breadcrumb.main'), href: MAIN_NAVIGATION_ROOT},
            {name: translate('vehicle-photo-category.breadcrumb.main'), href: LIST_NAVIGATION},
            {name: translate('vehicle-photo-category.breadcrumb.list')},
          ]}
          action={
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              >
                {translate('vehicle-photo-category.create')}
              </Button>
            </Stack>
          }
        />
