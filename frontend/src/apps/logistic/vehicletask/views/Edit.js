...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    updateData(id, data, {
      success: data => {
        enqueueSnackbar(translate('task.success.edit'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('task.error.edit'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('vehicle_task.title.edit')}</title>
      </Helmet>
