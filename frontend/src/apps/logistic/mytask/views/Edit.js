...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    updateData(id, data, {
      success: data => {
        enqueueSnackbar(translate('my-vehicle-task.success.edit'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('my-vehicle-task.error.edit'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('vehicle_my-vehicle-task.title.edit')}</title>
      </Helmet>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        ...
        <CustomBreadcrumbs
          heading={translate('vehicle_my-vehicle-task.title.edit')}
          links={[
            {name: translate('breadcrumb.main'), href: MAIN_NAVIGATION_ROOT},
            {name: translate('vehicle_my-vehicle-task.breadcrumb.main'), href: LIST_NAVIGATION},
            {name: translate('vehicle_my-vehicle-task.breadcrumb.edit')},
          ]}
        />
