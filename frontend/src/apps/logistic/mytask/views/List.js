...
export default function PageOne() {
  ...
  const onDelete = id => {
    confirmRef.current.open({
      title: translate('my-vehicle-task.delete.title'),
      content: convertNewlinesToBreaks(translate('my-vehicle-task.delete.description')),
      ...
      callback: (close) => {
        deleteData(id, {
          success: data => {
            enqueueSnackbar(translate('my-vehicle-task.success.delete'), {
              variant: 'success',
          ...
          error: e => {
            enqueueSnackbar(translate('my-vehicle-task.error.delete'), {
              variant: 'error',
              autoHideDuration: 5 * 1000
            })
            close()
  ...
  return (
    <>
      <Helmet>
        <title>{translate('my-vehicle-task.title.page')}</title>
      </Helmet>
