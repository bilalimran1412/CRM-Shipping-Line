...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    updateData(id, data, {
      success: data => {
        enqueueSnackbar(translate('vehicle-task.success.edit'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('vehicle-task.error.edit'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('vehicle-task.title.edit')}</title>
      </Helmet>
