...
export default function PageOne() {
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <RHFTextField
              ...
              placeholder={translate('shipment-company.filter.search')}
              InputProps={{
                startAdornment: (
  ...
  const onDelete = id => {
    confirmRef.current.open({
      title: translate('shipment-company.delete.title'),
      content: convertNewlinesToBreaks(translate('shipment-company.delete.description')),
      ...
      callback: (close) => {
        deleteData(id, {
          success: data => {
            enqueueSnackbar(translate('shipment-company.success.delete'), {
              variant: 'success',
          ...
          error: e => {
            enqueueSnackbar(translate('shipment-company.error.delete'), {
              variant: 'error',
              autoHideDuration: 5 * 1000
            })
            close()
  ...
  return (
    <>
      <Helmet>
        <title>{translate('shipment-company.title.page')}</title>
      </Helmet>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={translate('shipment-company.title.list')}
          links={[
            {name: translate('breadcrumb.main'), href: MAIN_NAVIGATION_ROOT},
            {name: translate('shipment-company.breadcrumb.main'), href: LIST_NAVIGATION},
            {name: translate('shipment-company.breadcrumb.list')},
          ]}
          action={
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              >
                {translate('shipment-company.create')}
              </Button>
            </Stack>
          }
        />
