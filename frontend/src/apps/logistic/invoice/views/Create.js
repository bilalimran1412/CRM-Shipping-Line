...
export default function UserEditPage() {
  ...
  const onSubmit = values => {
    ...
    createData(data, {
      success: data => {
        enqueueSnackbar(translate('invoice.success.create'), {
          variant: 'success',
      ...
      error: e => {
        enqueueSnackbar(translate('invoice.error.create'), {
          variant: 'error',
          autoHideDuration: 5 * 1000
        })
      }
  ...
  return (
    <>
      <Helmet>
        <title>{translate('invoice.title.create')}</title>
      </Helmet>
