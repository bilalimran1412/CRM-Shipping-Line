...
export default function PageOne() {
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <RHFTextField
              ...
              placeholder={translate('shipment-type.filter.search')}
              InputProps={{
                startAdornment: (
  ...
  const onDelete = id => {
    confirmRef.current.open({
      title: translate('shipment-type.delete.title'),
      content: convertNewlinesToBreaks(translate('shipment-type.delete.description')),
      ...
      callback: (close) => {
        deleteData(id, {
          success: data => {
            enqueueSnackbar(translate('shipment-type.success.delete'), {
              variant: 'success',
          ...
          error: e => {
            enqueueSnackbar(translate('shipment-type.error.delete'), {
              variant: 'error',
              autoHideDuration: 5 * 1000
            })
            close()
  ...
  return (
    <>
      <Helmet>
        <title>{translate('shipment-type.title.page')}</title>
      </Helmet>
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={translate('shipment-type.title.list')}
          links={[
            ...
            {name: translate('shipment-type.breadcrumb.list')},
          ]}
          action={
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              >
                {translate('shipment-type.create')}
              </Button>
            </Stack>
          }
        />
