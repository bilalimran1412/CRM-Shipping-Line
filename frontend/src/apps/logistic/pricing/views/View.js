...
export default function View() {
  ...
  return (
    <>
      <Helmet>
        <title>{translate('customer_invoice.title.view')}</title>
      </Helmet>

      <Container maxWidth={themeStretch ? false : 'lg'} className='relative-block'>
        <CustomBreadcrumbs
          heading={translate('customer_invoice.title.view')}
          links={[
            {name: translate('breadcrumb.main'), href: MAIN_NAVIGATION_ROOT},
            {name: translate('customer_invoice.breadcrumb.main'), href: LIST_NAVIGATION},
            {name: translate('customer_invoice.breadcrumb.view')},
          ]}
        />
                ...
                <Scrollbar>
                  <Table size={'small'}>
                    ...
                    <TableBody className={themeMode === 'dark' ? 'dark' : 'light'}>
                      ...
                      <TableRow>
                        <TableCell>{translate('customer_invoice.form.name')}</TableCell>
                        <TableCell>
                          <Typography variant="subtitle2">
                            {detail.name}
                          </Typography>
                      ...
                      <TableRow>
                        <TableCell>{translate('customer_invoice.form.date')}</TableCell>
                        <TableCell>
                          <Typography variant="subtitle2">
                            {detail.date}
                          </Typography>
                      ...
                      <TableRow>
                        <TableCell>{translate('customer_invoice.form.file')}</TableCell>
                        <TableCell>
                          <Typography variant="subtitle2">
                            {detail.file.name}
                          </Typography>
